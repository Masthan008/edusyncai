...
export default function TeacherDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'attendance' | 'grades' | 'assignments' | 'schedule' | 'sops'>('attendance');
  ...
  useEffect(() => {
    ...
    api.get('/sops', { signal: ac.signal }).then((res) => {
  ...
  useEffect(() => {
    if (location.state?.tab) {
      setActiveTab(location.state.tab as 'attendance' | 'grades' | 'assignments' | 'schedule' | 'sops');
    }
  }, [location.state?.tab]);
