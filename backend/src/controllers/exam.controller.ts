...
const calculateLetterGrade = (marks: number, max: number) => {
  ...
  if (pct >= 70) return { letter: 'B', point: 3.3 };
  if (pct >= 60) return { letter: 'C', point: 2.7 };
  if (pct >= 50) return { letter: 'D', point: 2.0 };
...
export const createExam = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const examRes = await query(
      `INSERT INTO exams (name, class_id, academic_year_id, start_date, end_date) 
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [name, class_id, academic_year_id, start_date, end_date]
    );
...
export const recordGrade = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const checkGrade = await query(
      'SELECT id FROM grades WHERE exam_id = $1 AND student_id = $2 AND subject_id = $3',
      [exam_id, student_id, subject_id]
    );

    let gradeRes;
    if (checkGrade.rowCount > 0) {
      gradeRes = await query(
        `UPDATE grades 
         SET marks_obtained = $1, max_marks = $2, grade_point = $3, letter_grade = $4, remarks = $5 
         WHERE exam_id = $6 AND student_id = $7 AND subject_id = $8 RETURNING *`,
        [marks_obtained, max_marks, point, letter, remarks || null, exam_id, student_id, subject_id]
      );
    } else {
      gradeRes = await query(
        `INSERT INTO grades (exam_id, student_id, subject_id, marks_obtained, max_marks, grade_point, letter_grade, remarks) 
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
        [exam_id, student_id, subject_id, marks_obtained, max_marks, point, letter, remarks || null]
      );
    }
