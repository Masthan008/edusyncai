...
export const getTeacherById = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const scheduleRes = await query(
      `SELECT tt.*, c.name as class_name, sec.name as section_name, s.name as subject_name 
       FROM timetables tt
...
export const createTeacher = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const teacherInsert = await query(
      `INSERT INTO teachers (id, first_name, last_name, phone, department_id, qualification)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [newUserId, first_name, last_name, phone || null, department_id || null, qualification || null]
    );
...
export const updateTeacher = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const updateRes = await query(
      ...
      [first_name, last_name, phone, department_id, qualification, status, id]
    );
