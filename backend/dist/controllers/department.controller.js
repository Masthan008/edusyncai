...
export const getDepartments = async (req, res, next) => {
    ...
    `;
        ...
        for (const d of depts) {
            const facultyCountRes = await query('SELECT COUNT(*)::int as count FROM teachers WHERE department_id = $1', [d.id]);
            d.faculty_count = facultyCountRes.rows[0]?.count || 0;
            const studentCountRes = await query(`SELECT COUNT(s.id)::int as count 
         FROM students s
         JOIN classes c ON s.class_id = c.id
         WHERE c.department_id = $1`, [d.id]);
            d.student_count = studentCountRes.rows[0]?.count || 0;
            // Fetch subject count
            const subjectCountRes = await query('SELECT COUNT(*)::int as count FROM subjects WHERE department_id = $1', [d.id]);
...
export const createDepartment = async (req, res, next) => {
    ...
    try {
        ...
        const resDept = await query('INSERT INTO departments (name, code, hod_id) VALUES ($1, $2, $3) RETURNING *', [name, code.toUpperCase(), hod_id || null]);
...
export const assignHOD = async (req, res, next) => {
    ...
    try {
        ...
        // Update
        const resDept = await query('UPDATE departments SET hod_id = $1 WHERE id = $2 RETURNING *', [hod_id, id]);
