...
export const getInstitutionOverview = async (req, res, next) => {
    try {
        ...
        const revenueTrendRes = await query(`SELECT TO_CHAR(payment_date, 'Mon YYYY') as month, 
       ...
       ORDER BY DATE_TRUNC('month', payment_date) ASC`);
        ...
        const revenueTrends = revenueTrendRes.rows.length > 0
            ? revenueTrendRes.rows.map((r) => ({ name: r.month, value: parseFloat(r.amount) }))
            : [
                { name: 'Jan', value: 4000 },
                { name: 'Feb', value: 3000 },
                { name: 'Mar', value: 2000 },
