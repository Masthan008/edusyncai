...
export const getFeeStructures = async (req, res, next) => {
    try {
        const feeRes = await query(`SELECT f.*, c.name as class_name, ay.name as academic_year_name
...
export const createFeeStructure = async (req, res, next) => {
    ...
    try {
        const resFee = await query(`INSERT INTO fee_structures (name, class_id, amount, due_date, academic_year_id) 
       VALUES ($1, $2, $3, $4, $5) RETURNING *`, [name, class_id || null, amount, due_date, academic_year_id]);
...
export const recordPayment = async (req, res, next) => {
    ...
    try {
        ...
        const payRes = await query(`INSERT INTO payments (student_id, fee_structure_id, amount_paid, payment_method, transaction_reference, status) 
       VALUES ($1, $2, $3, $4, $5, 'Paid') RETURNING *`, [student_id, fee_structure_id, amount_paid, payment_method, reference]);
        return res.status(201).json({
            success: true,
...
export const getStudentPayments = async (req, res, next) => {
    ...
    try {
        ...
        const duesRes = await query(`SELECT fs.*, 
              COALESCE((SELECT SUM(p.amount_paid) FROM payments p WHERE p.fee_structure_id = fs.id AND p.student_id = $1), 0) as paid_amount
       FROM fee_structures fs
       WHERE fs.class_id = $2 OR fs.class_id IS NULL`, [studentId, classId]);
        const invoices = duesRes.rows.map((row) => {
        ...
        const paymentsRes = await query(`SELECT p.*, fs.name as fee_name, fs.amount as total_fee_amount
       FROM payments p
       JOIN fee_structures fs ON p.fee_structure_id = fs.id
       WHERE p.student_id = $1
       ORDER BY p.payment_date DESC`, [studentId]);
        return res.status(200).json({
            success: true,
            data: {
                invoices,
...
export const getAccountantSummary = async (req, res, next) => {
    try {
        const revenueRes = await query("SELECT SUM(amount_paid) as total_revenue FROM payments WHERE status = 'Paid'");
        const paymentsCountRes = await query('SELECT COUNT(*) as tx_count FROM payments');
        // Fetch recent transaction history
        const recentRes = await query(`SELECT p.*, s.first_name || ' ' || s.last_name as student_name,
              fs.name as fee_name
       FROM payments p
