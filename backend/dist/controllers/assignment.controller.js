...
export const createAssignment = async (req, res, next) => {
    ...
    try {
        const assignRes = await query(`INSERT INTO assignments (title, description, subject_id, class_id, section_id, teacher_id, due_date, max_marks) 
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`, [title, description || null, subject_id, class_id, section_id, teacherId, due_date, max_marks]);
...
export const submitAssignment = async (req, res, next) => {
    ...
    try {
        ...
        // Upsert submission
        const checkSub = await query('SELECT id FROM assignment_submissions WHERE assignment_id = $1 AND student_id = $2', [assignment_id, studentId]);
        let subRes;
        if (checkSub.rowCount > 0) {
            subRes = await query(`UPDATE assignment_submissions 
         SET submission_date = CURRENT_TIMESTAMP, file_url = $1, student_notes = $2, status = $3
         WHERE assignment_id = $4 AND student_id = $5 RETURNING *`, [file_url || null, student_notes || null, status, assignment_id, studentId]);
        }
        else {
            subRes = await query(`INSERT INTO assignment_submissions (assignment_id, student_id, file_url, student_notes, status) 
         VALUES ($1, $2, $3, $4, $5) RETURNING *`, [assignment_id, studentId, file_url || null, student_notes || null, status]);
        }
        return res.status(200).json({
            success: true,
...
export const gradeSubmission = async (req, res, next) => {
    ...
    try {
        ...
        const gradeRes = await query(`UPDATE assignment_submissions 
       SET status = 'Graded', marks_obtained = $1, teacher_remarks = $2, graded_by = $3
       WHERE id = $4 RETURNING *`, [marks_obtained, teacher_remarks || null, teacherId, id]);
        return res.status(200).json({
            success: true,
