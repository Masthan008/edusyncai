...
export const login = async (req, res, next) => {
    ...
    try {
        ...
        const userRes = await query(`SELECT u.*, r.name as role_name 
       FROM users u 
       ...
       WHERE u.email = $1`, [email]);
        ...
        const accessToken = jwt.sign({ id: user.id, email: user.email, role: user.role_name }, config.jwtSecret, { expiresIn: '2h' });
...
export const getProfile = async (req, res, next) => {
    ...
    try {
        const userRes = await query(`SELECT u.id, u.email, u.is_active, r.name as role_name 
       FROM users u 
       JOIN roles r ON u.role_id = r.id 
       WHERE u.id = $1`, [userId]);
        if (userRes.rowCount === 0) {
