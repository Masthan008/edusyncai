...
export const submitAttendance = async (req, res, next) => {
    ...
    const teacherId = req.user?.id; // authenticated user (teacher/admin)
    try {
        // 1. Create or fetch attendance session
        const checkSession = await query(`SELECT id FROM attendance 
       WHERE date = $1 AND class_id = $2 AND section_id = $3 
         AND (subject_id = $4 OR (subject_id IS NULL AND $4 IS NULL))`, [date, class_id, section_id, subject_id || null]);
        let attendanceId;
        if (checkSession.rowCount > 0) {
            ...
            const newSession = await query(`INSERT INTO attendance (date, class_id, section_id, subject_id, taken_by) 
         VALUES ($1, $2, $3, $4, $5) RETURNING id`, [date, class_id, section_id, subject_id || null, teacherId]);
            attendanceId = newSession.rows[0].id;
        }
        // 2. Insert new records
        for (const rec of records) {
            await query(`INSERT INTO attendance_records (attendance_id, student_id, status, remarks) 
         VALUES ($1, $2, $3, $4)`, [attendanceId, rec.student_id, rec.status, rec.remarks || null]);
        }
...
export const getStudentAttendanceSummary = async (req, res, next) => {
    ...
    try {
        const recordsRes = await query(`SELECT ar.status, COUNT(*) as count 
       FROM attendance_records ar
       JOIN attendance a ON ar.attendance_id = a.id
       WHERE ar.student_id = $1
       GROUP BY ar.status`, [studentId]);
        ...
        stats.forEach((row) => {
            if (row.status === 'Present')
                ...
                excused = parseInt(row.count, 10);
        });
        const total = present + absent + late + excused;
        const rate = total > 0 ? ((present + late * 0.5) / total) * 100 : 100;
        // Fetch detailed subject attendance breakdown
        const subjectBreakdownRes = await query(`SELECT sub.id as subject_id, sub.name as subject_name, ar.status, COUNT(*) as count 
       FROM attendance_records ar
       JOIN attendance a ON ar.attendance_id = a.id
       JOIN subjects sub ON a.subject_id = sub.id
       WHERE ar.student_id = $1
       GROUP BY sub.id, sub.name, ar.status`, [studentId]);
        return res.status(200).json({
            success: true,
            data: {
