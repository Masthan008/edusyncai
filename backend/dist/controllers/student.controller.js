...
export const getStudentById = async (req, res, next) => {
    const { id } = req.params;
    try {
        const studentRes = await query(`SELECT s.*, u.email, c.name as class_name, sec.name as section_name,
...
export const admitStudent = async (req, res, next) => {
    const { first_name, last_name, email, password, admission_number, roll_number, class_id, section_id, parent_email, dob, gender, address, } = req.body;
    try {
        ...
        let parentId = null;
        if (parent_email) {
            const parentRes = await query(`SELECT p.id FROM parents p 
         JOIN users u ON p.id = u.id 
         WHERE u.email = $1`, [parent_email]);
        ...
        const userInsert = await query('INSERT INTO users (email, password_hash, role_id) VALUES ($1, $2, $3) RETURNING id', [email, passwordHash, studentRoleId]);
...
export const updateStudent = async (req, res, next) => {
    ...
    try {
        ...
        const updateRes = await query(`UPDATE students 
       ...
       WHERE id = $9 RETURNING *`, [first_name, last_name, roll_number, class_id, section_id, status, gender, address, id]);
        return res.status(200).json({
            success: true,
