...
export const getAcademicAssistantReply = async (req, res, next) => {
    const { question, studentId } = req.body;
    try {
        // 1. Gather student context to make response context-aware
        const studentRes = await query(`SELECT s.*, c.name as class_name, sec.name as section_name 
       FROM students s
...
export const getPerformanceInsights = async (req, res, next) => {
    ...
    try {
        const gradesRes = await query(`SELECT g.*, sub.name as subject_name 
       FROM grades g
       JOIN subjects sub ON g.subject_id = sub.id
       WHERE g.student_id = $1`, [studentId]);
        const grades = gradesRes.rows;
        const attRes = await query(`SELECT ar.status, COUNT(*) as count 
       ...
       GROUP BY ar.status`, [studentId]);
        ...
        if (grades.length > 0) {
            const totalMarks = grades.reduce((acc, val) => acc + parseFloat(val.marks_obtained), 0);
    ...
    try {
       ...
       WHERE g.student_id = $1`, [studentId]);
        const gradesText = gradesRes.rows
            .map((g) => `${g.subject_name}: ${g.marks_obtained}/${g.max_marks} (${g.letter_grade})`)
            .join(', ');
        const prompt = `
